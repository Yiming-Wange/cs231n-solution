...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2 * N):
            if i != k:
                denominator1 += torch.exp(sim(out[i], z_k) / tau)
            if i != k+N:
                denominator2 += torch.exp(sim(out[i], z_k_N) / tau)
        loss1 = -torch.log(torch.exp(sim(z_k, z_k_N)/tau) / denominator1) 
        loss2 = -torch.log(torch.exp(sim(z_k_N, z_k)/tau) / denominator2) 
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    zi = torch.linalg.norm(out_left, dim=1, keepdim=True) #(N, )
    zj = torch.linalg.norm(out_right, dim=1, keepdim=True)
    z = zi * zj
...
def compute_sim_matrix(out):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    sim_matrix = out.mm(out.T) / torch.linalg.norm(out, dim=1, keepdim=True).mm(torch.linalg.norm(out, dim=1, keepdim=True).T)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
