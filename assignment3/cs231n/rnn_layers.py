...
def rnn_step_backward(dnext_h, cache):
    ...
    d_tanh = 1 - next_h**2 # (N, H)
    dx = dnext_h * d_tanh @ Wx.T #(N, H) * (H, D)
    dprev_h = dnext_h * d_tanh @ Wh.T
    dWx = x.T @ (d_tanh * dnext_h) #(D, N) @ (N, H)
    dWh = prev_h.T @ (d_tanh * dnext_h)
    db = (d_tanh * dnext_h).sum(axis=0)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    #first iter
    x1 = x[:, 0, :]
    h_temp, cache_temp = rnn_step_forward(x1, h0, Wx, Wh, b)
    h[:, 0, :] = h_temp
    cache.append(cache_temp)
    # second-N iter
    for i in range(T-1):
        x_temp = x[:, i+1, :]
        h_temp, cache_temp = rnn_step_forward(x_temp, h_temp, Wx, Wh, b)
        cache.append(cache_temp)
        h[:, i+1, :] = h_temp
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        ...
        for j in (range(i, -1, -1)):
            cache_temp = cache[j]
            dx_temp, dnext_h, dWx_temp, dWh_temp, db_temp = rnn_step_backward(dnext_h, cache_temp)
            if dx_ is None:
                dx_ = np.zeros((N, T, D))
                dx_[:, j, :] = dx_temp
                dWx_, dWh_, db_ = dWx_temp, dWh_temp, db_temp
            else:
                dx_[:, j, :] += dx_temp
                dWx_ += dWx_temp
                dWh_ += dWh_temp
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    sig = x @ Wx + prev_h @ Wh + b #(N, 4H)
    H = int(sig.shape[1] / 4)
    i = sigmoid(sig[:, :H])
    f = sigmoid(sig[:, H:2*H])
    o = sigmoid(sig[:, 2*H:3*H])
    g = np.tanh(sig[:, 3*H:]) #(N, 4H)
    next_c = f * prev_c + i * g
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    di_ = di * i * (1 - i)
    dg_ = dg * (1 - g**2)
    dA = np.concatenate([di_, df_, do_, dg_], axis=1) #(N, 4H)
    db = np.sum(dA, axis=0)
    dx = dA @ Wx.T
    dprev_h = dA @ Wh.T
    dWx = x.T @ dA
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        X = x[:, t, :].squeeze()
        h0, prev_c, cache_ = lstm_step_forward(X, h0, prev_c, Wx, Wh, b)
        h[:, t, :] = h0
        cache.append(cache_)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
